...
const MessageContainer = () => {
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        ...
        setSelectedChatMessages([]); // Fallback to empty array on error
      }
    };
  ...
  // In your MessageContainer component

// In your MessageContainer component

const deleteMessage = async (messageId) => {
  try {
    const url = `/api/messages/delete-message/${messageId}`;
    console.log("Deleting message at:", `${HOST}${url}`,{withCredentials:true}); // Debugging URL
    const response = await apiClient.delete(url);
    console.log("Message deleted successfully:", response.data);
  } catch (error) {
    console.error("Delete message error:", error.response ? error.response.data : error.message);
  }
};
...
const editMessage = async (messageId, newContent) => {
  ...
  const pinMessage = async (messageId) => {
    try {
      ...
      if (response.status === 200) {
        // Update the pinned message in the UI
        setSelectedChatMessages((prevMessages) =>
          prevMessages.map((msg) =>
            ...
            {(isAdmin || isSender) && (
              <div className="flex gap-2 mt-2">
                ...
                <button
                  ...
                  //   if (newContent) editMessage(message._id, newContent);
                  // }}
